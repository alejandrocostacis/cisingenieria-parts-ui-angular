(function ()
{
  ...
  angular.module('app.settings').controller('WorkGroupDialogController', WorkGroupDialogController);

  function WorkGroupDialogController(MESSAGES, WorkGroupsService, ToastService, $mdDialog, item, callback, $log)
  {
    var vm = this;
    vm.cancel = cancel;
    vm.remove = remove;
    ...
    function update() {
      ...
      WorkGroupsService.update(item.id, tosave).then(function () {
    ...
    function create() {
      var tosave = getParametersForSave();
      WorkGroupsService.create(tosave).then(function () {
        ToastService.show(MESSAGES.savedSuccessfuly);
        close();
      }).catch(function (response) {
        ToastService.showFormError(MESSAGES.somethingHapped + ' ' + MESSAGES.imposibleToSave, ['name'], vm.form, response);
    ...
    function remove() {
      ...
      $mdDialog.show(confirm).then(function () {
        WorkGroupsService.destroy(item.id).then(function () {
