(function ()
{
  ...
  angular.module('app.settings').controller('SpecialityDialogController', SpecialityDialogController);

  function SpecialityDialogController(MESSAGES, SpecialitiesService, ToastService, $mdDialog, employeeAreaId, item, callback, $log)
  {
    var vm = this;
    vm.cancel = cancel;
    vm.remove = remove;
    ...
    function getParametersForSave() {
      ...
      if (vm.new){
        ...
        params.employee_area_id = employeeAreaId;
      } else {
    ...
    function update() {
      ...
      SpecialitiesService.update(item.id, tosave).then(function () {
    ...
    function create() {
      var tosave = getParametersForSave();
      SpecialitiesService.create(tosave).then(function () {
        ToastService.show(MESSAGES.savedSuccessfuly);
        close();
      }).catch(function (response) {
        ToastService.showFormError(MESSAGES.somethingHapped + ' ' + MESSAGES.imposibleToSave, ['name'], vm.form, response);
    ...
    function remove() {
      ...
      $mdDialog.show(confirm).then(function () {
        SpecialitiesService.destroy(item.id).then(function () {
