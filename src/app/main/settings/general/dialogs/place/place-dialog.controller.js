(function () {
  ...
  angular.module('app.settings').controller('PlaceDialogController', PlaceDialogController);

  function PlaceDialogController(MESSAGES, PlacesService, $scope, ToastService, item, $mdDialog, callback, $log) {
    var vm = this;
    vm.cancel = cancel;
    vm.remove = remove;
    ...
    function update() {
      ...
      PlacesService.update(item.id, tosave).then(function () {
    ...
    function create() {
      var tosave = getParametersForSave();
      PlacesService.create(tosave).then(function () {
        ToastService.show(MESSAGES.savedSuccessfuly);
        close();
      }).catch(function (response) {
        ToastService.showFormError(MESSAGES.somethingHapped + ' ' + MESSAGES.imposibleToSave, ['name'], vm.form, response);
    ...
    function remove() {
      ...
      $mdDialog.show(confirm).then(function () {
        PlacesService.destroy(item.id).then(function () {
