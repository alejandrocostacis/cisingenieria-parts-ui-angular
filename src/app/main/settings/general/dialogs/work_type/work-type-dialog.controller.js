(function ()
{
  ...
  angular.module('app.settings').controller('WorkTypeDialogController', WorkTypeDialogController);

  function WorkTypeDialogController(MESSAGES, WorkTypesService, ToastService, $mdDialog, item, callback, $log)
  {
    var vm = this;
    vm.cancel = cancel;
    vm.remove = remove;
    ...
    function update() {
      ...
      WorkTypesService.update(item.id, tosave).then(function () {
    ...
    function create() {
      var tosave = getParametersForSave();
      WorkTypesService.create(tosave).then(function () {
        ToastService.show(MESSAGES.savedSuccessfuly);
        close();
      }).catch(function (response) {
        ToastService.showFormError(MESSAGES.somethingHapped + ' ' + MESSAGES.imposibleToSave, ['name'], vm.form, response);
    ...
    function remove() {
      ...
      $mdDialog.show(confirm).then(function () {
        WorkTypesService.destroy(item.id).then(function () {
