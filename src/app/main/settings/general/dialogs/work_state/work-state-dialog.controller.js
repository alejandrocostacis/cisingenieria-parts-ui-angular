(function ()
{
  ...
  angular.module('app.settings').controller('WorkStateDialogController', WorkStateDialogController);

  function WorkStateDialogController(MESSAGES, WorkStatesService, ToastService, $mdDialog, item, callback, $log)
  {
    var vm = this;
    vm.cancel = cancel;
    vm.remove = remove;
    ...
    function getParametersForSave() {
      ...
      if (vm.new){
        ...
        params.color = vm.item.color != item.color? vm.item.color : undefined;
      }
    ...
    function update() {
      ...
      WorkStatesService.update(item.id, tosave).then(function () {
    ...
    function create() {
      var tosave = getParametersForSave();
      WorkStatesService.create(tosave).then(function () {
        ToastService.show(MESSAGES.savedSuccessfuly);
        close();
      }).catch(function (response) {
        ToastService.showFormError(MESSAGES.somethingHapped + ' ' + MESSAGES.imposibleToSave, ['name'], vm.form, response);
    ...
    function remove() {
      ...
      $mdDialog.show(confirm).then(function () {
        WorkStatesService.destroy(item.id).then(function () {
