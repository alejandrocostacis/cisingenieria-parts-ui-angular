(function () {
  ...
  angular.module('app.settings').controller('EmployeePositionDialogController', EmployeePositionDialogController);

  function EmployeePositionDialogController(MESSAGES, EmployeePositionsService, ToastService, $mdDialog, item, callback, $log) {
    var vm = this;
    vm.cancel = cancel;
    vm.remove = remove;
    ...
    function update() {
      ...
      EmployeePositionsService.update(item.id, tosave).then(function () {
    ...
    function create() {
      var tosave = getParametersForSave();
      EmployeePositionsService.create(tosave).then(function () {
        ToastService.show(MESSAGES.savedSuccessfuly);
        close();
      }).catch(function (response) {
        ToastService.showFormError(MESSAGES.somethingHapped + ' ' + MESSAGES.imposibleToSave, ['name'], vm.form, response);
    ...
    function remove() {
      ...
      $mdDialog.show(confirm).then(function () {
        EmployeePositionsService.destroy(item.id).then(function () {
