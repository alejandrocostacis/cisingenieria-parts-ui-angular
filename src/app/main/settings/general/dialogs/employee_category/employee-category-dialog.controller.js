(function () {
  ...
  angular.module('app.settings').controller('EmployeeCategoryDialogController', EmployeeCategoryDialogController);

  function EmployeeCategoryDialogController(MESSAGES, EmployeeCategoriesService, ToastService, $mdDialog, employeePositionId, item, callback, $log) {
    var vm = this;
    vm.cancel = cancel;
    vm.remove = remove;
    ...
    function getParametersForSave() {
      ...
      if (vm.new) {
        ...
        params.employee_position_id = employeePositionId;
      } else {
    ...
    function update() {
      ...
      EmployeeCategoriesService.update(item.id, tosave).then(function () {
    ...
    function create() {
      var tosave = getParametersForSave();
      EmployeeCategoriesService.create(tosave).then(function () {
        ToastService.show(MESSAGES.savedSuccessfuly);
        close();
      }).catch(function (response) {
        ToastService.showFormError(MESSAGES.somethingHapped + ' ' + MESSAGES.imposibleToSave, ['name'], vm.form, response);
    ...
    function remove() {
      ...
      $mdDialog.show(confirm).then(function () {
        EmployeeCategoriesService.destroy(item.id).then(function () {
