(function ()
{
  ...
  angular.module('app.settings').controller('EmployeeHourTypeDialogController', EmployeeHourTypeDialogController);

  function EmployeeHourTypeDialogController(MESSAGES, EmployeeHourTypesService, ToastService, $mdDialog, item, callback, $log)
  {
    var vm = this;
    vm.cancel = cancel;
    vm.remove = remove;
    ...
    function update() {
      ...
      EmployeeHourTypesService.update(item.id, tosave).then(function () {
    ...
    function create() {
      var tosave = getParametersForSave();
      EmployeeHourTypesService.create(tosave).then(function () {
        ToastService.show(MESSAGES.savedSuccessfuly);
        close();
      }).catch(function (response) {
        ToastService.showFormError(MESSAGES.somethingHapped + ' ' + MESSAGES.imposibleToSave, ['name'], vm.form, response);
    ...
    function remove() {
      ...
      $mdDialog.show(confirm).then(function () {
        EmployeeHourTypesService.destroy(item.id).then(function () {
